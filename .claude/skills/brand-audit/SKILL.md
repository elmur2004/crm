...
1. Grep for hex colors and `font-family` outside `branding/`, `src/themes/`, and token
   files — every hit is a finding. Sole exemption: the brand-token guard test
   `src/lib/brand-tokens.test.ts`, which exists to assert the canonical values
   (ADR-015).
...
4. ByteForce: only the five palette values (+ functional danger red `#C0392B`,
   ADR-014); violet is `#53449B` (ADR-001); no emoji in App A UI strings; Lama Sans
   via token only.
