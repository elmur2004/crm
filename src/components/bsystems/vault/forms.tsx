...
function FormModal({
  ...
  const submit = async (fd: FormData) => {
    ...
    const ok = row
      ? await run(`/api/b-systems/vault/forms/${row.id}`, "PATCH", body, onClose)
      : await run("/api/b-systems/vault/forms", "POST", body, onClose);
...
function SheetModal({ row, onClose }: { row: VaultSheetRow | null; onClose: () => void }) {
  ...
  const submit = async (fd: FormData) => {
    if (row) {
      ...
      await run(`/api/b-systems/vault/sheets/${row.id}`, "PATCH", body, onClose);
      return;
    }
    fd.set("storage", storage);
    await run("/api/b-systems/vault/sheets", "POST", fd, onClose);
  };
...
function DocumentModal({ row, onClose }: { row: VaultDocumentRow | null; onClose: () => void }) {
  ...
  const submit = async (fd: FormData) => {
    if (row) {
      ...
      await run(`/api/b-systems/vault/documents/${row.id}`, "PATCH", body, onClose);
      return;
    }
    await run("/api/b-systems/vault/documents", "POST", fd, onClose);
  };
...
function EmployeeModal({ row, onClose }: { row: VaultEmployeeRow | null; onClose: () => void }) {
  ...
  const submit = async (fd: FormData) => {
    ...
    if (row) await run(`/api/b-systems/vault/employees/${row.id}`, "PATCH", body, onClose);
    else await run("/api/b-systems/vault/employees", "POST", body, onClose);
  };
  ...
  return (
    <span className="inline-flex items-center gap-2 flex-wrap">
      ...
      <button
        ...
        onClick={() =>
          run(`/api/b-systems/vault/employees/${row.id}`, "PATCH", { active: !row.active })
        }
      >
...
function TaskModal({
  ...
  const submit = async (fd: FormData) => {
    ...
    if (row) await run(`/api/b-systems/vault/tasks/${row.id}`, "PATCH", body, onClose);
    else await run("/api/b-systems/vault/tasks", "POST", body, onClose);
  };
  ...
  return (
    <div className="modal-overlay" onClick={close}>
      <form
        ...
        onSubmit={(e) => {
          e.preventDefault();
          void run(`/api/b-systems/vault/tasks/${taskId}/complete`, "POST", buildFormData(), close);
        }}
      >
        <div className="modal-head">
          <div>
        ...
        <div className="modal-foot">
          ...
          <span className="flex gap-2">
            <button
              ...
              disabled={busy}
              onClick={() =>
                void run(`/api/b-systems/vault/tasks/${taskId}/result`, "POST", buildFormData(), close)
              }
            >
              {t(vault.saveResult)}
            </button>
  ...
  return (
    <span className="inline-flex items-center gap-2">
      ...
      <button
        ...
        onClick={() => run(`/api/b-systems/vault/tasks/${taskId}/reopen`, "POST", {})}
      >
        {t(vault.reopenTask)}
      </button>
    </span>
