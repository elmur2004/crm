...
export default async function VaultArchivePage() {
  ...
  const sections: Array<{ label: Msg; rows: ArchivedRow[] }> = [
    {
      ...
      rows: forms.map((f) => ({
        ...
        restoreUrl: `/api/b-systems/vault/forms/${f.id}/archive`,
      })),
    },
    {
      ...
      rows: sheets.map((s) => ({
        ...
        restoreUrl: `/api/b-systems/vault/sheets/${s.id}/archive`,
      })),
    },
    {
      ...
      rows: documents.map((d) => ({
        ...
        restoreUrl: `/api/b-systems/vault/documents/${d.id}/archive`,
      })),
    },
    {
      ...
      rows: tasks.map((task) => ({
        ...
        restoreUrl: `/api/b-systems/vault/tasks/${task.id}/archive`,
      })),
    },
  ];
  ...
  return (
    <div className="space-y-5">
      <VaultHead tab="archive" locale={locale} title={vault.archiveTitle} sub={vault.archiveSub} />
