...
import {
  ...
  EditTaskButton,
  ReopenTaskButton,
} from "@/components/bsystems/vault/forms";
...
export default async function VaultTasksPage({
  ...
  return (
    <div className="space-y-5">
      <VaultHead tab="tasks" locale={locale} title={vault.tasksTitle} sub={vault.tasksSub} />
      ...
      {cards.length === 0 ? (
        ...
        <div className="ecard-grid">
          {cards.map((c) => (
            <Link
              key={c.id}
              href={employeeFilter === c.id ? "/b-systems/vault/tasks" : `/b-systems/vault/tasks?employee=${c.id}`}
                  ...
                  <tr key={task.id}>
                    ...
                    <td>
                      <span className="inline-flex items-center gap-2 flex-wrap">
                        ...
                        />
                        <ArchiveButton
                          postUrl={`/api/b-systems/vault/tasks/${task.id}/archive`}
