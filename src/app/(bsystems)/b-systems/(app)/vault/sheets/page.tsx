...
export default async function VaultSheetsPage({
  ...
  return (
    <div className="space-y-5">
      <VaultHead tab="sheets" locale={locale} title={vault.sheetsTitle} sub={vault.sheetsSub} />
                    ...
                    <tr key={s.id}>
                      ...
                      <td>
                        <span className="inline-flex items-center gap-2 flex-wrap">
                          ...
                          />
                          <ReplaceFileButton
                            postUrl={`/api/b-systems/vault/sheets/${s.id}/file`}
                            hint={t(vault.sheetFileTypes)}
                            accept=".xlsx,.xls,.csv"
                            label={current ? t(vault.replaceFile) : t(vault.uploadFile)}
                          />
                          <ArchiveButton
                            postUrl={`/api/b-systems/vault/sheets/${s.id}/archive`}
